...
async function processConsolidation(groupId, startDate, endDate) {
    ...
    if (!sucesso) {
        log(`❌ Falha na consolidação de vendas (grupo ${groupId})`, 'workerConsolidation');
        return;
    }
    ...
    await callPHP('registerJobExecution', {
        ...
        system_unit_id: groupId, // aqui você usa o id do grupo como referência
        custom_code: groupId,
    ...
    log(`✅ Consolidação concluída em ${executionTime} minutos (grupo ${groupId})`, 'workerConsolidation');
}
