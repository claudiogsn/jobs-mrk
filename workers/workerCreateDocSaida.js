...
async function processDocSaida({ group_id, data } = {}) {
  ...
  log(`🔄 Processando movimentação de saída para o grupo ${groupId} na data ${dataRef}`, 'workerCreateDocSaida');
  ...
  for (const unidade of unidades) {
    ...
    try {
      const inicio = Date.now();
        log(`🔄 Iniciando processamento para unidade ${system_unit_id}`, 'workerCreateDocSaida');
